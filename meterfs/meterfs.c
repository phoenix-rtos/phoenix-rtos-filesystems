...
static int meterfs_updateOneFile(fileheader_t *f, meterfs_ctx_t *ctx)
{
	/* This function assumes meterfs_powerCtrl(1, ctx) to be already called */
...
static int meterfs_updateFileInfo(fileheader_t *f, meterfs_ctx_t *ctx)
{
	...
	return meterfs_updateOneFile(f, ctx);
}
...
int meterfs_resetFile(file_t *f, meterfs_ctx_t *ctx)
{
	...
	return meterfs_updateOneFile(&hdr, ctx);
}
