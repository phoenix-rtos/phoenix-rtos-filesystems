...
static int check_node_data(struct jffs2_sb_info *c, struct jffs2_tmp_dnode_info *tn)
{
	...
	if (jffs2_is_writebuffered(c)) {
		...
		if (adj >= tn->csize) {
			dbg_readinode("no need to check node at %#08x, data length %d, data starts at %#08x - it has already been checked.\n",
	...
	dbg_readinode("check node at %#08x, data length %d, partial CRC %#08x, correct CRC %#08x, data starts at %#08x, start checking from %#08x - %d bytes.\n",
	...
	if (!err && retlen < len) {
		JFFS2_WARNING("MTD point returned len too short: %zu instead of %d.\n", retlen, tn->csize);
	...
	if (!pointed) {
		...
		if (err) {
			JFFS2_ERROR("can not read %d bytes from 0x0x%x, error code: %d.\n", len, ofs, err);
			goto free_out;
		}
	...
	if (crc != tn->data_crc) {
		JFFS2_NOTICE("wrong data CRC in data node at 0x0x%x: read %#08x, calculated %#08x.\n",
...
static int check_tn_node(struct jffs2_sb_info *c, struct jffs2_tmp_dnode_info *tn)
{
	...
	dbg_readinode("check node #%x-#%x, phys offs %#08x\n",
...
static struct jffs2_tmp_dnode_info *jffs2_lookup_tn(os_tree_root_t *tn_root, uint32_t offset)
{
	os_tree_node_t *next;
				...
				struct jffs2_tmp_dnode_info *tn)
{
	...
	dbg_readinode("insert fragment #%x-#%x, ver %d at 0x%x\n", tn->fn->ofs, fn_end, tn->version, ref_offset(tn->fn->raw));
	...
	if (this) {
		...
		dbg_readinode("'this' found #%x-#%x (%s)\n", this->fn->ofs, this->fn->ofs + this->fn->size, this->fn ? "data" : "hole");
	}
	...
	{
		os_tree_node_t *parent;
		os_tree_node_t **link = &rb_node(&rii->tn_root);
...
static void ver_insert(os_tree_root_t *ver_root, struct jffs2_tmp_dnode_info *tn)
{
	os_tree_node_t **link = &rb_node(ver_root);
	os_tree_node_t *parent = NULL;
				      ...
				      struct jffs2_readinode_info *rii)
{
	...
	os_tree_root_t ver_root = OS_TREE_ROOT;
	uint32_t high_ver = 0;
...
static void jffs2_free_tmp_dnode_info_list(os_tree_root_t *list)
{
...
static struct jffs2_raw_node_ref *jffs2_first_valid_node(struct jffs2_raw_node_ref *ref)
{
	while (ref && ref->next_in_ino) {
		if (!ref_obsolete(ref))
			return ref;
		dbg_noderef("node at 0x0x%x is obsoleted. Ignoring.\n", ref_offset(ref));
		ref = ref->next_in_ino;
	}
	return NULL;
}
				...
				struct jffs2_readinode_info *rii)
{
	...
	if (ref_flags(ref) == REF_UNCHECKED) {
		...
		if (unlikely(PAD((rd->nsize + sizeof(*rd))) != PAD(je32_to_cpu(rd->totlen)))) {
			JFFS2_ERROR("illegal nsize in node at %#08x: nsize %#02x, totlen #%x\n",
			     ...
			     struct jffs2_readinode_info *rii)
{
	...
	if (ref_flags(ref) == REF_UNCHECKED) {
		...
		if (unlikely(je32_to_cpu(rd->offset) > je32_to_cpu(rd->isize)) ||
			...
			JFFS2_WARNING("inode node header CRC is corrupted at #%x\n", ref_offset(ref));
	...
	dbg_readinode2("dnode @0x%x: ver %d, offset #%x, dsize #%x, csize #%x\n",
...
static inline int read_unknown(struct jffs2_sb_info *c, struct jffs2_raw_node_ref *ref, struct jffs2_unknown_node *un)
{
	...
	if (ref_flags(ref) == REF_UNCHECKED) {
		JFFS2_ERROR("REF_UNCHECKED but unknown node at %#08x\n",
			    ref_offset(ref));
		JFFS2_ERROR("Node is {%04x,%04x,0x%x,0x%x}. Please report this error.\n",
...
static int read_more(struct jffs2_sb_info *c, struct jffs2_raw_node_ref *ref,
		     int needed_len, int *rdlen, char *buf)
{
	...
	if (err) {
		JFFS2_ERROR("can not read %d bytes from 0x0x%x, "
			"error code: %d.\n", to_read, offs, err);
		return err;
	}
				 ...
				 struct jffs2_readinode_info *rii)
{
	...
	if (!valid_ref && f->inocache->ino != 1)
		JFFS2_WARNING("Eep. No valid nodes for ino #%d.\n", f->inocache->ino);
	while (valid_ref) {
		...
		*/
		...
		if (je32_to_cpu(node->u.hdr_crc) != crc32(0, node, sizeof(node->u)-4)) {
			JFFS2_NOTICE("Node header CRC failed at %#08x. {%04x,%04x,0x%x,0x%x}\n",
	...
	dbg_readinode("nodes of inode #%d were read, the highest version is %d, latest_mctime %d, mctime_ver %d.\n",
					...
					struct jffs2_raw_inode *latest_node)
{
	...
	dbg_readinode("ino #%d pino/nlink is %d\n", f->inocache->ino,
	...
	if (ret) {
		JFFS2_ERROR("cannot read nodes for ino %d, returned error is %d\n", f->inocache->ino, ret);
	...
	if (ret) {
		JFFS2_ERROR("Failed to build final fragtree for inode #%d: error %d\n",
			    f->inocache->ino, ret);
		if (f->inocache->state == INO_STATE_READING)
			jffs2_set_inocache_state(c, f->inocache, INO_STATE_CHECKEDABSENT);
		jffs2_free_tmp_dnode_info_list(&rii.tn_root);
	...
	if (unlikely(!rii.latest_ref)) {
		...
		if (f->inocache->ino != 1) {
			JFFS2_WARNING("no data nodes found for ino #%d\n", f->inocache->ino);
			if (!rii.fds) {
				if (f->inocache->state == INO_STATE_READING)
					jffs2_set_inocache_state(c, f->inocache, INO_STATE_CHECKEDABSENT);
				return -EIO;
	...
	if (crc != je32_to_cpu(latest_node->node_crc)) {
		JFFS2_ERROR("CRC failed for read_inode of inode %d at physical location 0x%x\n",
	...
	case S_IFREG:
		...
		if (new_size != je32_to_cpu(latest_node->isize)) {
			JFFS2_WARNING("Truncating ino #%d to %d bytes failed because it only had %d bytes to start with!\n",
...
// 			}
	...
	case S_IFCHR:
		...
		if (f->metadata) {
			JFFS2_ERROR("Argh. Special inode #%d with mode 0%o had metadata node\n",
			       f->inocache->ino, jemode_to_cpu(latest_node->mode));
			jffs2_do_clear_inode(c, f);
			return -EIO;
		}
		if (!frag_first(&f->fragtree)) {
			JFFS2_ERROR("Argh. Special inode #%d with mode 0%o has no fragments\n",
			       f->inocache->ino, jemode_to_cpu(latest_node->mode));
			jffs2_do_clear_inode(c, f);
			return -EIO;
		}
		...
		if (frag_next(frag_first(&f->fragtree))) {
			JFFS2_ERROR("Argh. Special inode #%d with mode 0x%x had more than one node\n",
			       f->inocache->ino, jemode_to_cpu(latest_node->mode));
			...
			uint32_t ino, struct jffs2_raw_inode *latest_node)
{
 ...
 retry_inocache:
	...
	if (f->inocache) {
		...
		case INO_STATE_GC:
			/* If it's in either of these states, we need
			   ...
			   put it back. */
			dbg_readinode("waiting for ino #%d in state %d\n", ino, f->inocache->state);
		...
		case INO_STATE_PRESENT:
			...
			JFFS2_ERROR("Eep. Trying to read_inode #%d when it's already in state %d!\n", ino, f->inocache->state);
	...
	if (!f->inocache) {
		JFFS2_ERROR("requestied to read an nonexistent ino %d\n", ino);
		return -ENOENT;
	}
...
int jffs2_do_crccheck_inode(struct jffs2_sb_info *c, struct jffs2_inode_cache *ic)
{
	...
	struct jffs2_inode_info *f = jffs2_alloc_inode_info();
	int ret;
