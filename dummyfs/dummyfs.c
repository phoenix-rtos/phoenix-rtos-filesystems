...
static inline int dummyfs_root(oid_t *oid)
{
	return oid->id == 0;
}
...
{
	return dummyfs_device(ctx, oid) ? dev_find(ctx, oid) : object_get(ctx, oid->id);
}
...
int dummyfs_lookup(void *ctx, oid_t *dir, const char *name, oid_t *res, oid_t *dev)
{
	...
	while (name[len] != '\0') {
		...
		end = strchr(completePath + len, '/');
		if (end != NULL) {
			*end = 0;
		}

		if (!strcmp(completePath + len, ".")) {
			*res = d->oid;
			len++;
			continue;
		}
		else if (!strcmp(completePath + len, "..")) {
			...
			err = dir_find(d, completePath + len, res);
			len += err;
			object_unlock(fs, d);
			object_put(fs, d);
...
int dummyfs_setattr(void *ctx, oid_t *oid, int type, long long attr, const void *data, size_t size)
{
	...
	switch (type) {
		...
		case (atDev):
			if (data != NULL && size == sizeof(oid_t) && S_ISDIR(o->mode) && dummyfs_device(fs, &o->dev) == 0) {
				object_unlock(fs, o);
				ret = dev_create(fs, (oid_t *)data, o);
				object_lock(fs, o);
			}
			else {
				ret = -EINVAL;
...
int dummyfs_mount(void **ctx, const char *data, unsigned long mode, oid_t *root)
{
	...
	if (data != NULL) {
		abspath = resolve_path(data, NULL, 1, 0);
		if (lookup(abspath, &fs->parent, NULL) < EOK) {
			free(abspath);
			dev_cleanup(fs);
			object_cleanup(fs);
			resourceDestroy(fs->mutex);
