...
#include <phoenix/posix.h>

#include "ext2.h"
#include "block.h"
#include "file.h"

static int ext2_search_block(ext2_t *info, void *data, const char *name, uint8_t len, id_t *res)
{
	ext2_de_t *de = (ext2_dir_entry_t *)data;
	uintptr_t addr = (uintptr_t)data;

	while (addr - (uintptr_t)data < info->blocksz && de->entrysz) {
		if (de->len == len && !strncmp(de->name, name, len)) {
			*res = de->inode;
			return EOK;
		}

		addr += de->entrysz;
		de = (ext2_dir_entry_t *)(void *)addr;
	}
...
int dir_find(ext2_obj_t *d, const char *name, uint32_t len, id_t *resId)
{
	uint32_t offs = 0;
	void *data;
	int err = 0;
	...
	while (offs < d->inode->size) {
		ext2_read_internal(d, offs, data, d->f->block_size);
		offs += d->f->block_size;
...
int dir_add(ext2_obj_t *d, const char *name, size_t len, uint16_t mode, id_t *id)
{
	uint32_t rec_len = 0;
	uint32_t offs = 0;
	void *data = NULL;
	...
	data = malloc(d->f->block_size);
	ext2_read_internal(d, d->inode->size ? d->inode->size - d->f->block_size : 0, data, d->f->block_size);
	...
	ext2_write_unlocked(d->f, &d->id, d->inode->size ? d->inode->size - d->f->block_size : 0, data, d->f->block_size);
...
int dir_remove(ext2_obj_t *d, const char *name, size_t len)
{
	uint32_t offs = 0;
	uint32_t block_offs;
	int found = 0;
	...
	ext2_write_unlocked(d->f, &d->id, offs, data, d->f->block_size);
	free(data);
	return EOK;
}

int dir_is_empty(ext2_obj_t *d)
{
	uint32_t offs = 0;
	void *data = NULL;
	ext2_dir_entry_t *dentry;
	...
	ext2_read(d->f, &d->id, 0, data, d->f->block_size);
