...
int liblfs_handler(void *fdata, msg_t *msg)
{
	...
	switch (msg->type) {
		case mtCreate:
			msg->o.err = ro ? -EROFS : liblfs_create(fdata, &msg->oid, msg->i.data, &msg->o.create.oid, msg->i.create.mode, msg->i.create.type, &msg->i.create.dev);
			break;

		case mtDestroy:
			msg->o.err = ro ? -EROFS : liblfs_destroy(fdata, &msg->oid);
			break;
		...
		case mtWrite:
			msg->o.err = ro ? -EROFS : liblfs_write(fdata, &msg->oid, msg->i.io.offs, msg->i.data, msg->i.size);
			break;

		case mtTruncate:
			msg->o.err = ro ? -EROFS : liblfs_truncate(fdata, &msg->oid, msg->i.io.len);
			break;
		...
		case mtLink:
			msg->o.err = ro ? -EROFS : liblfs_link(fdata, &msg->oid, msg->i.data, &msg->i.ln.oid);
			break;

		case mtUnlink:
			msg->o.err = ro ? -EROFS : liblfs_unlink(fdata, &msg->oid, msg->i.data);
			break;
		...
		size_t writeSize, unsigned long mode)
{
	...
	cfg->ph.maxCachedObjects = LIBLFS_N_CACHED_OBJECTS;
...
static int liblfs_mountCommon(lfs_t **infoOut, const struct lfs_config *cfg, unsigned int port, bool format)
{
		...
		ssize_t (*write)(id_t, off_t, const char *, size_t), void **fdata)
{
	...
	int err = liblfs_setConfig(cfg, NULL, 64 * 1024 * 1024, 4096, 16, mode);
	if (err < 0) {
		free(cfg);
		return err;
	}
...
{
	/* The function may not be defined; if it is not, assume it's not necessary and return 0 */
	storage_t *strg = (storage_t *)c->context;
...
int liblfs_storage_mount(storage_t *strg, storage_fs_t *fs, const char *data, unsigned long mode, oid_t *root)
{
	...
	err = liblfs_mountCommon((lfs_t **)&fs->info, cfg, root->port, strcmp("format", data) == 0);
	if (err < 0) {
		free(cfg);
		return err;
	}
...
{
	int err = liblfs_mountCommon((lfs_t **)fs_handle, cfg, root->port, 0);
	if (err < 0) {
		return err;
	}
