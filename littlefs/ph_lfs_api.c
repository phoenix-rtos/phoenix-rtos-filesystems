...
static void ph_lfs_addToLRU(lfs_t *lfs, ph_lfs_lru_t *obj)
{
	...
	LIST_ADD(&lfs->ph.phLfsObjects, obj);
	lfs->ph.nPhLfsObjects++;
	if (lfs->ph.nPhLfsObjects > lfs->cfg->ph.maxCachedObjects) {
...
static int ph_lfs_file_rawsync(lfs_t *lfs, lfs_file_t *file)
{
	...
	if (((file->flags & LFS_F_DIRTY) != 0) && !lfs_pair_isnull(file->m.pair)) {
		...
		uint8_t modTimeLE[sizeof(ph_lfs_time_t)];
		ph_lfs_attrToLE(time(NULL), modTimeLE, sizeof(ph_lfs_time_t));
		err = lfs_dir_commit(
				lfs,
				&file->m,
				LFS_MKATTRS(
						{ tag, buffer },
						{ LFS_MKTAG_IF(lfs->cfg->ph.useMTime != 0, LFS_TYPE_PH_ATTR_MTIME, file->id, sizeof(modTimeLE)), modTimeLE }));
		if (err != 0) {
			file->flags |= LFS_F_ERRED;
			return err;
		}
...
static int ph_lfs_commitPayload(lfs_t *lfs, ph_lfs_newFileData *d, const char *name, size_t nlen, uint16_t mode, oid_t *dev, id_t *result)
{
	...
	uint8_t creationTimeLE[sizeof(ph_lfs_time_t)];
	ph_lfs_attrToLE(time(NULL), creationTimeLE, sizeof(ph_lfs_time_t));
	uint32_t modeLE = lfs_tole32(mode);
...
int ph_lfs_getattrAll(lfs_t *lfs, id_t phId, struct _attrAll *attrs)
{
	...
	/* mode & type */
	attrs->mode.err = ph_lfs_getSimpleAttr(lfs, obj, &attrs->mode.val, LFS_TYPE_PH_ATTR_MODE, sizeof(uint32_t));
	attrs->type.err = attrs->mode.err;
	if (attrs->mode.err >= 0) {
		attrs->type.val = ph_lfs_modeToType(attrs->mode.val);
	}

	attrs->uid.err = ph_lfs_getSimpleAttr(lfs, obj, &attrs->uid.val, LFS_TYPE_PH_ATTR_UID, sizeof(uint32_t));
	attrs->gid.err = ph_lfs_getSimpleAttr(lfs, obj, &attrs->gid.val, LFS_TYPE_PH_ATTR_GID, sizeof(uint32_t));
	...
	if (obj->extrasType == EXTRAS_TYPE_FILE) {
		/* Get cached file size - this is necessary for correct functioning */
		attrs->size.val = lfs_file_rawsize(lfs, obj->extras);
		attrs->size.err = EOK;
	}
	else {
		attrs->size.err = ph_lfs_getSimpleAttr(
				lfs, obj, &attrs->size.val, LFS_TYPE_STRUCT, sizeof(uint32_t));
	}
	...
	attrs->cTime.err = ph_lfs_getSimpleAttr(lfs, obj, &attrs->cTime.val, LFS_TYPE_PH_ATTR_CTIME, sizeof(ph_lfs_time_t));
	...
	attrs->mTime.err = ph_lfs_getSimpleAttr(lfs, obj, &attrs->mTime.val, LFS_TYPE_PH_ATTR_MTIME, sizeof(ph_lfs_time_t));
	...
	attrs->aTime.err = ph_lfs_getSimpleAttr(lfs, obj, &attrs->aTime.val, LFS_TYPE_PH_ATTR_ATIME, sizeof(ph_lfs_time_t));
...
static int ph_lfs_dir_getinfo(lfs_t *lfs, lfs_mdir_t *dir, uint16_t id, struct dirent *info, size_t maxNameLength)
{
	...
	const lfs_stag_t modeTag = lfs_dir_get(lfs, dir, LFS_MKTAG(0x7ff, 0x3ff, 0),
			LFS_MKTAG(LFS_TYPE_PH_ATTR_MODE, id, sizeof(modeLE)), &modeLE);
	...
	if (modeTag < 0) {
		...
		uint32_t mode = lfs_fromle32(modeLE);
		switch (mode & S_IFMT) {
