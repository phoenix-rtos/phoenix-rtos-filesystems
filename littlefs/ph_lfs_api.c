...
#else
static const int TRACE_FIXUP_ENABLE = 0;
#endif
...
static int ph_lfs_setSimpleAttr(lfs_t *lfs, ph_lfs_lru_t *obj, long long attr, uint16_t attrType);
static int ph_lfs_attrDeserialize(const uint8_t attrSerial[8], long long *attr, lfs_tag_t tag);
static size_t ph_lfs_attrSerialize(long long attr, uint8_t attrSerial[8], uint16_t attrType);


void ph_lfs_bumpLastPhId(lfs_t *lfs, id_t found)
{
	if (found > lfs->ph.lastFileId) {
		lfs->ph.lastFileId = found;
	}
}


static int ph_lfs_getNextPhId(lfs_t *lfs, id_t *phId)
{
	static const id_t maxId = (id_t)~0ULL;
	if (lfs->ph.lastFileId == maxId) {
		return LFS_ERR_NOSPC;
	}

	lfs->ph.lastFileId++;
	*phId = lfs->ph.lastFileId;
	return 0;
}
...
{
	LFS_ASSERT(lfs->ph.lastFileId > LFS_ROOT_PHID);
	if (lfs->ph.lastFileId == unusedId) {
		lfs->ph.lastFileId--;
	}
}
...
{
	lib_rbInit(&lfs->ph.phIdTree, ph_lfs_comparePhID, NULL);
	lfs->ph.phLfsObjects = NULL;
	lfs->ph.nPhLfsObjects = 0;
	lfs->ph.port = port;
	lfs->ph.lastFileId = LFS_ROOT_PHID;
	lfs->ph.openDirs = NULL;
	lfs->ph.initialScan = true;
...
static lfs_stag_t ph_lfs_findById(lfs_t *lfs, lfs_mdir_t *dir, const lfs_block_t *pair, uint16_t id, id_t phId)
{
	dir->tail[0] = pair[0];
	dir->tail[1] = pair[1];
	struct ph_lfs_matchPhId_data matchData = { lfs, ph_lfs_idToLE(phId) };
	while (true) {
...
static lfs_stag_t ph_lfs_scanForId(lfs_t *lfs, lfs_mdir_t *dir, id_t phId)
{
	...
	TRACE("scanning for ID %" PRIu32, (uint32_t)phId);
...
static void ph_lfs_removeLRU(lfs_t *lfs, ph_lfs_lru_t *obj)
{
	...
	lfs->ph.nPhLfsObjects--;
	LIST_REMOVE(&lfs->ph.phLfsObjects, obj);
	lib_rbRemove(&lfs->ph.phIdTree, &obj->phIdNode);
}
...
static ph_lfs_lru_t *ph_lfs_getLRU(lfs_t *lfs, id_t phId)
{
	...
	ph_lfs_lru_t *obj = lib_treeof(ph_lfs_lru_t, phIdNode, lib_rbFind(&lfs->ph.phIdTree, &find.phIdNode));
	if (obj != NULL) {
		/* Shift to back */
		LIST_REMOVE(&lfs->ph.phLfsObjects, obj);
		LIST_ADD(&lfs->ph.phLfsObjects, obj);
	}
...
{
	lib_rbInsert(&lfs->ph.phIdTree, &obj->phIdNode);
	LIST_ADD(&lfs->ph.phLfsObjects, obj);
	lfs->ph.nPhLfsObjects++;
	if (lfs->ph.nPhLfsObjects > lfs->cfg->ph.objectEvictThreshold) {
		/* The newly inserted object is at the end of the list
		 * and also shouldn't be removed because we are about to use it */
		for (ph_lfs_lru_t *i = lfs->ph.phLfsObjects; i != obj; i = i->next) {
			if (ph_lfs_objIsEvictable(i)) {
				TRACE2("evicting obj %" PRIu32, (uint32_t)i->phId);
...
{
	for (lfs_dir_t **p = &lfs->ph.openDirs; *p; p = &(*p)->nextDir) {
		if (*p == elem) {
			*p = (*p)->nextDir;
			break;
		}
...
{
	elem->nextDir = lfs->ph.openDirs;
	lfs->ph.openDirs = elem;
}
...
static lfs_stag_t ph_lfs_getPhID(lfs_t *lfs, const lfs_mdir_t *dir, uint16_t id, id_t *phId)
{
	...
	if (tag == LFS_ERR_NOENT) {
		...
		ph_lfs_lru_t *obj = lfs->ph.phLfsObjects;
		if (obj == NULL) {
			return tag;
		}
		...
		} while (obj != lfs->ph.phLfsObjects);
	}

	if (tag >= 0) {
		*phId = ph_lfs_idToLE(*phId);
	}
...
static int ph_lfs_file_rawsync(lfs_t *lfs, lfs_file_t *file)
{
	...
	if (((file->flags & LFS_F_DIRTY) != 0) && !lfs_pair_isnull(file->m.pair)) {
		...
		if ((file->flags & LFS_F_INLINE) != 0) {
			...
			tag = LFS_MKTAG(LFS_TYPE_INLINESTRUCT, file->id, file->ctz.size);
		}
		else {
			ctz = file->ctz;
			lfs_ctz_tole32(&ctz);
			buffer = &ctz;
			tag = LFS_MKTAG(LFS_TYPE_CTZSTRUCT, file->id, sizeof(ctz));
		}

		uint8_t mtimeSerial[8] = { 0 };
		size_t mtimeSize = 0;
		if (lfs->cfg->ph.useMTime != 0) {
			mtimeSize = ph_lfs_attrSerialize(time(NULL), mtimeSerial, LFS_TYPE_PH_ATTR_MTIME);
			LFS_ASSERT(mtimeSize != 0);
		}
		...
		err = lfs_dir_commit(
				...
				LFS_MKATTRS(
						{ tag, buffer },
						{ LFS_MKTAG_IF(lfs->cfg->ph.useMTime != 0, LFS_TYPE_PH_ATTR_MTIME, file->id, mtimeSize), mtimeSerial }));
		if (err != 0) {
			file->flags |= LFS_F_ERRED;
			return err;
		}
...
static int ph_lfs_dir_rawopen(lfs_t *lfs, ph_lfs_lru_t *obj)
{
	...
	if (obj->id == 0x3ff) {
		...
		err = ph_lfs_readDirPair(lfs, &dir->m, obj->id, pair);
		if (err != 0) {
...
static int ph_lfs_openObj(lfs_t *lfs, id_t phId, ph_lfs_lru_t **objOut, bool specificType, enum extrasType expectedType)
{
	...
	if (obj->extrasType == EXTRAS_TYPE_DIR) {
		...
		if ((lfs->cfg->ph.useATime != 0) && (lfs->cfg->ph.readOnly == 0)) {
			(void)ph_lfs_setSimpleAttr(lfs, obj, time(NULL), LFS_TYPE_PH_ATTR_ATIME);
		}
...
static int ph_lfs_commitPayload(lfs_t *lfs, ph_lfs_newFileData *d, const char *name, size_t nlen, uint16_t mode, oid_t *dev, id_t *result)
{
	...
	id_t phId;
	int err = ph_lfs_getNextPhId(lfs, &phId);
	if (err != 0) {
		return err;
	}
	...
	uint8_t ctimeSerial[8] = { 0 };
	size_t ctimeSize = 0;
	if ((lfs->cfg->ph.useCTime != 0) || (lfs->cfg->ph.useMTime != 0)) {
		ctimeSize = ph_lfs_attrSerialize(time(NULL), ctimeSerial, LFS_TYPE_PH_ATTR_CTIME);
		LFS_ASSERT(ctimeSize != 0);
	}

	uint8_t modeSerial[8];
	size_t modeSize = ph_lfs_attrSerialize(mode, modeSerial, LFS_TYPE_PH_ATTR_MODE);
	LFS_ASSERT(modeSize != 0);
	id_t phIdLE = ph_lfs_idToLE(phId);
	err = lfs_dir_commit(lfs, d->parent,
			LFS_MKATTRS(
					{ LFS_MKTAG(LFS_TYPE_CREATE, d->newId, 0), NULL },
					{ nameTag, name },
					{ phIdTag, &phIdLE },
					{ LFS_MKTAG(LFS_TYPE_PH_ATTR_MODE, d->newId, modeSize), modeSerial },
					{ LFS_MKTAG_IF(ctimeSize != 0, LFS_TYPE_PH_ATTR_CTIME, d->newId, ctimeSize), ctimeSerial },
...
int ph_lfs_create(lfs_t *lfs, id_t parentPhId, const char *name, uint16_t mode, oid_t *dev, id_t *result)
{
	...
	if (lfs->cfg->ph.useMTime != 0) {
		(void)ph_lfs_setSimpleAttr(lfs, parentObj, time(NULL), LFS_TYPE_PH_ATTR_MTIME);
	}
	...
	if (id == 0x3ff) {
		...
		*res = obj->phId;
		...
		if (phIdTag == LFS_ERR_NOENT) {
			...
			int err = ph_lfs_getNextPhId(lfs, res);
			if (err != 0) {
				return err;
			}
			...
			TRACE2("lookup res %" PRIu32, (uint32_t)*res);
...
static int ph_lfs_attrDeserialize(const uint8_t attrSerial[8], long long *attr, lfs_tag_t tag)
{
	lfs_size_t attrSize = lfs_tag_size(tag);
	uint16_t attrType = lfs_tag_type3(tag);
	if ((attrSize == 0) || (attrSize != ph_lfs_attrSize(attrType))) {
		TRACE("Invalid tag type %x size: got %u expected %u", attrType, attrSize, ph_lfs_attrSize(attrType));
		return LFS_ERR_NOATTR;
	}

	switch (attrType) {
		case LFS_TYPE_PH_ATTR_MODE: /* Fall-through */
		case LFS_TYPE_PH_ATTR_UID:  /* Fall-through */
		case LFS_TYPE_PH_ATTR_GID: {
			uint32_t val;
			memcpy(&val, attrSerial, sizeof(val));
#if defined(PH_LFS_LE_NOOP)
			*attr = (long long)val;
#else
			*attr = (long long)__builtin_bswap32(val);
#endif
			return 0;
		}

		case LFS_TYPE_PH_ATTR_CTIME: /* Fall-through */
		case LFS_TYPE_PH_ATTR_MTIME: /* Fall-through */
		case LFS_TYPE_PH_ATTR_ATIME: {
			ph_lfs_time_t val;
			memcpy(&val, attrSerial, sizeof(val));
#if defined(PH_LFS_LE_NOOP)
			*attr = (long long)val;
#else
			*attr = (long long)__builtin_bswap64(val);
#endif
			return 0;
		}
...
static size_t ph_lfs_attrSerialize(long long attr, uint8_t attrSerial[8], uint16_t attrType)
{
	switch (attrType) {
		...
		case LFS_TYPE_PH_ATTR_GID: {
			...
			val = (uint32_t)attr;
#else
			val = (uint32_t)__builtin_bswap32(attr);
#endif
			memcpy(attrSerial, &val, sizeof(val));
			return sizeof(val);
		}
		...
		case LFS_TYPE_PH_ATTR_ATIME: {
			ph_lfs_time_t val;
#if defined(PH_LFS_LE_NOOP)
			val = (ph_lfs_time_t)attr;
#else
			val = (ph_lfs_time_t)__builtin_bswap64(attr);
#endif
			memcpy(attrSerial, &val, sizeof(val));
			return sizeof(val);
		}
...
static int ph_lfs_getSimpleAttr(lfs_t *lfs, ph_lfs_lru_t *obj, long long *attr, uint32_t attrType)
{
	lfs_mdir_t mdir;
	const lfs_block_t *pair = (obj->phId == LFS_ROOT_PHID) ? lfs->root : obj->parentBlock;
	int err = lfs_dir_fetch(lfs, &mdir, pair);
	if (err != 0) {
	...
	lfs_stag_t tag;
	do {
		tag = lfs_dir_get(lfs, &mdir,
				LFS_MKTAG(mask, 0x3ff, 0),
				LFS_MKTAG(attrType, obj->id, sizeof(tagData)),
				&tagData);

		/* User trying to get a userattr, but it's not found on disk - apply fallbacks */
		if ((attrType != LFS_TYPE_STRUCT) && (tag == LFS_ERR_NOENT)) {
	...
	if (lfs_tag_type3(tag) == LFS_TYPE_CTZSTRUCT) {
		...
		*attr = 2 * lfs->cfg->block_size;
	}
	else {
		return ph_lfs_attrDeserialize(tagData.attr, attr, tag);
	}
...
static long long ph_lfs_modeToType(long long attr)
{
...
static int ph_lfs_getSize(lfs_t *lfs, ph_lfs_lru_t *obj, long long *attr)
{
	...
	return ph_lfs_getSimpleAttr(lfs, obj, attr, LFS_TYPE_STRUCT);
}
...
int ph_lfs_getattr(lfs_t *lfs, id_t phId, int type, long long *attr)
{
	...
	/* First check for types which are not stored as userattr */
	switch (type) {
		case atLinks:
		...
		case atSize:
			err = ph_lfs_getSize(lfs, obj, attr);
			if (err < 0) {
				return err;
			}
			...
			if (type == atBlocks) {
				...
				*attr = (size + (lfs->cfg->block_size - 1)) / lfs->cfg->block_size;
			}
	...
	switch (type) {
		...
		case atMode: attrType = LFS_TYPE_PH_ATTR_MODE; break;
		case atUid: attrType = LFS_TYPE_PH_ATTR_UID; break;
		case atGid: attrType = LFS_TYPE_PH_ATTR_GID; break;
		case atCTime: attrType = LFS_TYPE_PH_ATTR_CTIME; break;
		case atMTime: attrType = LFS_TYPE_PH_ATTR_MTIME; break;
		case atATime: attrType = LFS_TYPE_PH_ATTR_ATIME; break;
		default: return LFS_ERR_INVAL;
	}

	int ret = ph_lfs_getSimpleAttr(lfs, obj, attr, attrType);
	if (ret < 0) {
		return ret;
	}
...
int ph_lfs_getattrAll(lfs_t *lfs, id_t phId, struct _attrAll *attrs)
{
	...
	attrs->size.err = ph_lfs_getSize(lfs, obj, &attrs->size.val);
	if (attrs->size.err >= 0) {
		uint32_t size = (uint32_t)attrs->size.val;
		attrs->blocks.val = (size + (lfs->cfg->block_size - 1)) / lfs->cfg->block_size;
	}
...
static int ph_lfs_setSimpleAttr(lfs_t *lfs, ph_lfs_lru_t *obj, long long attr, uint16_t attrType)
{
	...
	uint8_t attrSerial[8];
	uint32_t attrSize = ph_lfs_attrSerialize(attr, attrSerial, attrType);
	LFS_ASSERT(attrSize != 0);
	return lfs_dir_commit(lfs, &mdir, LFS_MKATTRS({ LFS_MKTAG(attrType, obj->id, attrSize), attrSerial }));
}


static int ph_lfs_setDev(lfs_t *lfs, id_t phId, const void *data, size_t size)
{
	...
	if ((dev.port == lfs->ph.port) && (dev.id == phId)) {
...
{
	if (type == atDev) {
		/* Device OIDs are stored alongside objects in memory, not stored on disk - handle them separately */
	...
	uint32_t attrType;
	/* Attributes that get written as userattr tags */
	switch (type) {
		case atMode: {
			long long currentAttr;
			int ret = ph_lfs_getSimpleAttr(lfs, obj, &currentAttr, LFS_TYPE_PH_ATTR_MODE);
			if (ret < 0) {
				return ret;
			}
	...
	return ph_lfs_setSimpleAttr(lfs, obj, attr, attrType);
}
...
static int ph_lfs_dir_getinfo(lfs_t *lfs, lfs_mdir_t *dir, uint16_t id, struct dirent *info, size_t maxNameLength)
{
	...
	const lfs_stag_t modeTag = lfs_dir_get(lfs, dir, LFS_MKTAG(0x7ff, 0x3ff, 0),
			LFS_MKTAG(LFS_TYPE_PH_ATTR_MODE, id, sizeof(modeSerial)), modeSerial);

	if ((modeTag < 0) || (ph_lfs_attrDeserialize(modeSerial, &mode, modeTag) < 0)) {
...
static int ph_lfs_dir_rawread(lfs_t *lfs, lfs_dir_t *dir, struct dirent *info, size_t maxNameLength)
{
	...
	while (true) {
		...
		int err = ph_lfs_dir_getinfo(lfs, &dir->m, dir->id, info, maxNameLength);
...
int ph_lfs_readdir(lfs_t *lfs, id_t phId, size_t offs, struct dirent *dent, size_t size)
{
	...
	if (dirObj->extrasType != EXTRAS_TYPE_DIR) {
		...
		TRACE("invalid %d (id %" PRIu32 ")", dirObj->extrasType, (uint32_t)dirObj->phId);
		return LFS_ERR_INVAL;
	}
...
static int ph_lfs_rawrename(lfs_t *lfs, ph_lfs_lru_t *parentObj, const char *name, ph_lfs_lru_t *sourceObj)
{
	...
	bool prevIsDir = lfs_tag_type3(prevtag) == LFS_TYPE_DIR;
	bool oldIsDir = lfs_tag_type3(oldtag) == LFS_TYPE_PHID_DIR;
	...
	if (!samepair || prevtag != LFS_ERR_NOENT) {
		/* Detecting move between directories could be performed in fixup logic, but it's easier and more performant
		 * to just do it here - we know where the file will end up. */
		sourceObj->parentBlock[0] = newcwd.pair[0];
		sourceObj->parentBlock[1] = newcwd.pair[1];
		sourceObj->id = newid;
	}
	...
	err = lfs_dir_commit(lfs, &newcwd,
			LFS_MKATTRS(
					...
					{ LFS_MKTAG(oldIsDir ? LFS_TYPE_DIR : LFS_TYPE_REG, newid, nlen), name },
					{ LFS_MKTAG(LFS_FROM_MOVE, newid, oldid), &oldcwd },
					{ LFS_MKTAG_IF(samepair, LFS_TYPE_DELETE, newoldid, 0), NULL }));
	if (err != 0) {
		if (!samepair || prevtag != LFS_ERR_NOENT) {
			/* Roll back our update */
			sourceObj->parentBlock[0] = oldcwd.pair[0];
			sourceObj->parentBlock[1] = oldcwd.pair[1];
			sourceObj->id = oldid;
		}
...
int ph_lfs_rename(lfs_t *lfs, id_t dir, id_t source, const char *newName)
{
	...
	err = ph_lfs_rawrename(lfs, parentObj, newName, sourceObj);
	if (lfs->cfg->ph.useMTime != 0) {
		(void)ph_lfs_setSimpleAttr(lfs, parentObj, time(NULL), LFS_TYPE_PH_ATTR_MTIME);
	}
...
{
	while (lfs->ph.phLfsObjects != NULL) {
		(void)ph_lfs_closeObj(lfs, (ph_lfs_lru_t *)lfs->ph.phLfsObjects, true);
	}
...
static int ph_lfs_objectFixupOnCommit(lfs_t *lfs, const lfs_mdir_t *dir, ph_lfs_lru_t *obj, const struct lfs_mattr *ops, int nOps)
{
	...
	TRACE_FIXUP("fixing obj %x %x %d (ph %" PRIu32 ") ", obj->parentBlock[0], obj->parentBlock[1], obj->id, (uint32_t)obj->phId);
	bool wasDeleted;
	...
	if (wasDeleted) {
		...
		if (f != NULL) {
			f->m.pair[0] = LFS_BLOCK_NULL;
			f->m.pair[1] = LFS_BLOCK_NULL;
		}
...
{
	obj->m = *dir;
	uint16_t previousId = obj->id;
	bool wasDeleted = ph_lfs_updateId(&obj->id, ops, nOps);
	if (wasDeleted) {
		obj->m.pair[0] = LFS_BLOCK_NULL;
		obj->m.pair[1] = LFS_BLOCK_NULL;
		return 0;
	}

	/* Apply the same change to pos as we did to ID */
	obj->pos += (int)obj->id - (int)previousId;

	while ((obj->id >= dir->count) && dir->split) {
		if (obj->id == 0 && dir->count == 0) {
			/* TODO: not sure why this is necessary, but otherwise this loop can go on forever
			 * and this fix seems to work */
			break;
		}

		/* we split and id is on tail now */
		obj->id -= dir->count;
		int err = lfs_dir_fetch(lfs, &obj->m, dir->tail);
		if (err != 0) {
			return err;
		}
	}
...
int ph_lfs_updateOnCommit(lfs_t *lfs, const lfs_block_t oldpair[2], const lfs_mdir_t *dir, const struct lfs_mattr *ops, int nOps)
{
	...
	for (lfs_dir_t *obj = lfs->ph.openDirs; obj != NULL; obj = obj->nextDir) {
		if (lfs_pair_cmp(obj->m.pair, oldpair) == 0) {
...
void ph_lfs_updateOnRelocate(lfs_t *lfs, const lfs_block_t oldPair[2], const lfs_block_t newPair[2])
{
	if (lfs->ph.phLfsObjects != NULL) {
		...
		do {
			obj = obj->next;
			if (lfs_pair_cmp(obj->parentBlock, oldPair) == 0) {
				TRACE_FIXUP("relocating %" PRIu32 "\n", (uint32_t)obj->phId);
				obj->parentBlock[0] = newPair[0];
				obj->parentBlock[1] = newPair[1];
				if (obj->extrasType == EXTRAS_TYPE_FILE) {
					lfs_file_t *f = (lfs_file_t *)obj->extras;
					f->m.pair[0] = newPair[0];
					f->m.pair[1] = newPair[1];
				}
			}
		} while (obj != lfs->ph.phLfsObjects);
	}

	for (lfs_dir_t *dir = lfs->ph.openDirs; dir != NULL; dir = dir->nextDir) {
		if (lfs_pair_cmp(dir->m.pair, oldPair) == 0) {
			dir->m.pair[0] = newPair[0];
			dir->m.pair[1] = newPair[1];
		}
...
{
	if (!lfs->ph.largeInlineOpened || (lfs->ph.phLfsObjects == NULL)) {
		return 0;
	}
	...
	lfs->ph.largeInlineOpened = false;
	return 0;
}
