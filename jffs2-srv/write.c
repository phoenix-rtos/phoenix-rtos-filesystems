...
int jffs2_do_new_inode(struct jffs2_sb_info *c, struct jffs2_inode_info *f,
		       jmode_t mode, uint32_t datasize, struct jffs2_raw_inode *ri)
{
	...
	ri->totlen = cpu_to_je32(sizeof(*ri) + datasize);
	ri->isize = ri->dsize = ri->csize = cpu_to_je32(datasize);
...
int jffs2_write_inode_range(struct jffs2_sb_info *c, struct jffs2_inode_info *f,
			    struct jffs2_raw_inode *ri, char *buf,
				...
				os_time_t *f_mtime, os_time_t *f_ctime, os_time_t *f_atime)
{
	...
	retry:
		...
		*f_mtime = *f_ctime = *f_atime = jffs2_to_os_time(je32_to_cpu(ri->atime));
		mutex_unlock(&f->sem);
		jffs2_complete_reservation(c);
		if (!datalen) {
...
/* The OS-specific code fills in the metadata in the jffs2_raw_inode for us, so that
   we don't have to go digging in struct inode or its equivalent. It should set:
   mode, uid, gid, (starting)isize, atime, ctime, mtime */
int jffs2_write0_inode_range(struct jffs2_sb_info *c, struct jffs2_inode_info *f, struct jffs2_raw_inode *ri, uint32_t offset, uint32_t writelen)
{
	int ret = 0;
	struct jffs2_full_dnode *fn;
	uint32_t alloclen;
	...
	ri->ino = cpu_to_je32(f->inocache->ino);
	ri->version = cpu_to_je32(++f->highest_version);
	ri->isize = cpu_to_je32(max(je32_to_cpu(ri->isize), offset + writelen));
	ri->offset = cpu_to_je32(offset);
	ri->csize = cpu_to_je32(0);
	ri->dsize = cpu_to_je32(writelen);
	ri->compr = JFFS2_COMPR_ZERO;
	ri->node_crc = cpu_to_je32(crc32(0, ri, sizeof(*ri)-8));
	ri->data_crc = cpu_to_je32(crc32(0, NULL, 0));

	fn = jffs2_write_dnode(c, f, ri, NULL, 0, ALLOC_NORETRY);
					...
					const char *data, int datalen, __u8 type)
{
	 ...
	 */
	ret = jffs2_reserve_space(c, je32_to_cpu(ri->totlen), &alloclen, ALLOC_NORMAL,
				JFFS2_SUMMARY_INODE_SIZE);
...
int jffs2_do_change_meta(struct jffs2_sb_info *c, struct jffs2_inode_info *f, struct jffs2_raw_inode *ri, const char *data, int datalen)
{
	struct jffs2_full_dnode *new_metadata, *old_metadata;
	uint32_t alloclen;
	int ret;

	ret = jffs2_reserve_space(c, sizeof(*ri) + datalen, &alloclen, ALLOC_NORMAL,
				JFFS2_SUMMARY_INODE_SIZE);
	jffs2_dbg(1, "%s(): reserved 0x%x bytes\n", __func__, alloclen);
	if (ret)
		return ret;
	...
	ri->magic = cpu_to_je16(JFFS2_MAGIC_BITMASK);
	ri->nodetype = cpu_to_je16(JFFS2_NODETYPE_INODE);
	ri->totlen = cpu_to_je32(sizeof(*ri) + datalen);
	ri->hdr_crc = cpu_to_je32(crc32(0, ri, sizeof(struct jffs2_unknown_node)-4));
	...
	ri->csize = ri->dsize = cpu_to_je32(datalen);
	ri->compr = JFFS2_COMPR_NONE;
	ri->data_crc = cpu_to_je32(crc32(0, data, datalen));
	ri->node_crc = cpu_to_je32(crc32(0, ri, sizeof(*ri)-8));

	new_metadata = jffs2_write_dnode(c, f, ri, data, datalen, ALLOC_NORMAL);
...
int jffs2_do_unlink(struct jffs2_sb_info *c, struct jffs2_inode_info *dir_f,
		    ...
		    uint32_t time, os_time_t *dir_mtime, os_time_t *dir_ctime, os_time_t *dir_atime)
{
	struct jffs2_raw_dirent *rd;
	struct jffs2_full_dirent *fd;
	uint32_t alloclen;
	...
	*dir_ctime = *dir_mtime = *dir_atime = jffs2_to_os_time(time);
	mutex_unlock(&dir_f->sem);
	...
	if (dead_f && dead_f->inocache) {
		...
		if (S_ISDIR(JFFS2_F_I_MODE(dead_f))) {
			while (dead_f->dents) {
