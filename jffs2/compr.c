...
static os_spin_t jffs2_compressor_list_lock;

/* Available compressors are on this list */
static os_compressor_list_t jffs2_compressor_list;
 ...
 * compression mode.
 */
static int jffs2_selected_compress(u8 compr, char *data_in,
		char **cpage_out, u32 *datalen, u32 *cdatalen)
{
...
int jffs2_register_compressor(struct jffs2_compressor *comp)
{
	...
	list_for_each_entry(this, &jffs2_compressor_list, list) {
		if (this->priority < comp->priority) {
			list_add_before(comp, list, this);
			goto out;
		}
	}
...
int jffs2_unregister_compressor(struct jffs2_compressor *comp)
{
	...
	list_del(comp, list, &jffs2_compressor_list);
...
{
	INIT_LIST_HEAD(&jffs2_compressor_list);
	spin_init(&jffs2_compressor_list_lock);
