		     ...
		     int ofs, int len)
{
	...
	if (ret) {
		...
		pr_warn("Error reading node from 0x%x: %d\n",
			ref_offset(fd->raw), ret);
		return ret;
	}
	if (readlen != sizeof(*ri)) {
		jffs2_free_raw_inode(ri);
		pr_warn("Short read from 0x%x: wanted 0x%x bytes, got 0x%x\n",
			ref_offset(fd->raw), sizeof(*ri), readlen);
		return -EIO;
	}
	crc = crc32(0, ri, sizeof(*ri)-8);

	jffs2_dbg(1, "Node read from %x: node_crc %x, calculated CRC %x. dsize %x, csize %x, offset %x, buf %p\n",
		  ref_offset(fd->raw), je32_to_cpu(ri->node_crc),
		  crc, je32_to_cpu(ri->dsize), je32_to_cpu(ri->csize),
		  je32_to_cpu(ri->offset), buf);
	if (crc != je32_to_cpu(ri->node_crc)) {
		pr_warn("Node CRC %x != calculated CRC %x for node at %x\n",
	...
	*/
	...
	if (crc != je32_to_cpu(ri->data_crc)) {
		pr_warn("Data CRC %x != calculated CRC %x for node at %x\n",
			je32_to_cpu(ri->data_crc), crc, ref_offset(fd->raw));
		ret = -EIO;
		goto out_decomprbuf;
	}
	jffs2_dbg(2, "Data CRC matches calculated CRC %x\n", crc);
	if (ri->compr != JFFS2_COMPR_NONE) {
			   ...
			   char *buf, uint32_t offset, uint32_t len)
{
	...
	jffs2_dbg(1, "%s(): ino #%d, range 0x%x-0x%x\n",
	...
	while(offset < end) {
		...
		if (unlikely(!frag || frag->ofs > offset ||
			...
			if (frag && frag->ofs > offset) {
				jffs2_dbg(1, "Eep. Hole in ino #%d fraglist. frag->ofs = 0x%x, offset = 0x%x\n",
			...
			jffs2_dbg(1, "Reading %d-%d from node at 0x%x (%d)\n",
				  frag->ofs+fragofs,
