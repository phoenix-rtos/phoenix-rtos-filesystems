...
#define ROFS_HDR_AES_MAC   32
#define ROFS_HEADER_SIZE   64

_Static_assert(AES_BLOCKLEN <= ROFS_HEADER_SIZE - ROFS_HDR_AES_MAC, "AES_MAC does not fit the rofs header");
...
int rofs_init(struct rofs_ctx *ctx, rofs_devRead_t devRead, unsigned long imageAddr, const uint8_t *key)
{
	...
	memcpy(target_mac, ctx->buf + ROFS_HDR_AES_MAC, AES_BLOCKLEN);
